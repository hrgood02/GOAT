<?php
    ...
    $id = $_SESSION["id"];
    $result = mysqli_query($dbc,"SELECT * from holds NATURAL JOIN media NATURAL JOIN mediaInfo 
    ...
    if($result){
        while($row = mysqli_fetch_array($result)){
            ...
            echo '<h5 class="card-title">'.$row['title'].'</h5>
                  ...
                  <p class="card-text"><small class="text-muted">Estimated Date Available: '.$row['expectedDate'].'</small></p>
                </div>
              </div>
            </div>';
